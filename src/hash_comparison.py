...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def compare_hashes(self):
        ...
        if self.md5_radio_button.isChecked():
            original_file_hash = subprocess.run(['md5',FILE_TO_HASH], stdout=subprocess.PIPE, text = True).stdout
            original_file_hash = original_file_hash.split('=', 1)[-1].strip() # strip extraneous text from output and whitespace
            self.computed_hash_line.setText(original_file_hash)
        elif self.sha1_radio_button.isChecked():
            original_file_hash = subprocess.run(['shasum', FILE_TO_HASH], stdout=subprocess.PIPE, text = True).stdout
            original_file_hash = original_file_hash.split(' ', 1)[0].strip()
            self.computed_hash_line.setText(original_file_hash)
        elif self.sha256_radio_button.isChecked():
            original_file_hash = subprocess.run(['shasum', '-a', '256', FILE_TO_HASH], stdout=subprocess.PIPE, text = True).stdout
            original_file_hash = original_file_hash.split(' ', 1)[0].strip()
            self.computed_hash_line.setText(original_file_hash)
        else:
